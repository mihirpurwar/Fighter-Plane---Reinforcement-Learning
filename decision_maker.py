...
class Food:
    ...
    def move(self):
        ...
        #print(da)
        #da[0] = da[0] + (random.randint(10,90) - self.position[0])/dist
        #da[1] = da[1] + (random.randint(10,60) - self.position[1])/dist
...
class Prey:
    ...
    def move(self):
        ...
        if PARA.speed_optimization:
            self.speedOptimization(all_range_preys[3])
        if PARA.boundary_check:
            self.boundaryCheck()
        if self.qlearn.chosen_action == PreyAction.MoveTowardsFood:
            #print('hey')
            self.moveTowardsFood(foods)
            if PreyState.PreyInRepulsion in currentState or PreyState.PredatorDetected in currentState:
            ...
            if PreyState.PredatorDetected in currentState:
                self.qlearn.doQLearning(self.get_reward(4),currentState)
            else:
                if len(preys_repulsion)!=0:
                    self.qlearn.doQLearning(self.get_reward(1),currentState)
...
#        di_angle = math.degrees(math.atan(da[1]/da[0]))
    ...
    def speedOptimization(self,s_preys):
        # y=mx+b
        m = (-1)*(self.velocity[0]/self.velocity[1])
        b = (self.position[1]+(self.velocity[1]*2))-(m*(self.position[0]+(self.velocity[0]*2)))

        t = [self.position[0]+(self.velocity[0]*2), self.position[1]+(self.velocity[1]*2)]
        
        visible_preys1 = []
        visible_preys2 = []
        shortest_distance1 = 1500
        shortest_distance2 = 1500
        nearest_prey1 = None
        nearest_prey2 = None
        for prey in s_preys:
            y1 = (m*prey.position[0])+b
            t1 = [prey.position[0]+(prey.velocity[0]*2), prey.position[1]+(prey.velocity[1]*2)]
            y2 = (m*t1[0])+b
            if (self.velocity[0] > 0 and self.velocity[1] > 0) or (self.velocity[0] < 0 and self.velocity[1] > 0):
                if (prey.position[1]-y1) > 0:
                    visible_preys1.append(prey)
                    dist1 = distance_calc(prey.position, t)
                    if dist1 < shortest_distance1:
                        shortest_diatance1 = dist1
                        nearest_prey1 = prey
                dist2 = distance_calc(t1, t)
                if (t1[1]-y2) > 0 and dist2 < 2:
                    visible_preys2.append(prey)
                    if dist2 < shortest_distance2:
                        shortest_diatance2 = dist2
                        nearest_prey2 = prey
                    
            elif (self.velocity[0] < 0 and self.velocity[1] < 0) or (self.velocity[0] > 0 and self.velocity[1] < 0):
                if (prey.position[1]-y1) < 0:
                    visible_preys1.append(prey)
                    dist1 = distance_calc(prey.position,self.position)
                    if dist1 < shortest_distance1:
                        shortest_diatance1 = dist1
                        nearest_prey1 = prey
                dist2 = distance_calc(t1, t)
        ...
        if nearest_prey2 is not None and self.speed > 0.05:
            #print('nn: ' + str(nearest_prey.id))
            if len(visible_preys2) > 0:
                self.speed = nearest_prey2.speed*0.9
            else:
                self.speed = self.maxspeed
                #self.speed = nearest_prey2.speed*1.2
        elif nearest_prey1 is not None and self.speed > 0.05:
            #print('nn: ' + str(nearest_prey.id))
            if len(visible_preys1) > 3:
                self.speed = nearest_prey1.speed*0.95
            else:
                self.speed = self.maxspeed
                #self.speed = nearest_prey1.speed*1.2
        else:
            self.speed = self.maxspeed
        '''
        if self.id == 12:
            #print('nn: ' + str(nearest_prey.id))
            print('len.: ' + str(len(visible_preys1)) + '  :  len.2: ' + str(len(visible_preys1)))
        '''
